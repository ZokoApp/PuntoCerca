...
function renderStore(store) {
  ...
  document.getElementById("storeDescription").innerText =
    store.description || "";
  // REDES SOCIALES
  const socialContainer = document.getElementById('storeSocialLinks');
  if (socialContainer) {
    const links = [
      {
        url: store.instagram_url,
        label: 'Instagram',
        color: '#e91e63',
        bg: '#fce4ec',
        svg: `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="#e91e63"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>`
      },
      {
        url: store.tiktok_url,
        label: 'TikTok',
        color: '#000000',
        bg: '#f3f4f6',
        svg: `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="#000000"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1V9.01a6.27 6.27 0 00-.79-.05 6.34 6.34 0 00-6.34 6.34 6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.33-6.34V8.69a8.18 8.18 0 004.78 1.52V6.75a4.85 4.85 0 01-1.01-.06z"/></svg>`
      },
      {
        url: store.facebook_url,
        label: 'Facebook',
        color: '#1877f2',
        bg: '#e3f2fd',
      ...
      },
      {
        ...
        color: '#374151',
        bg: '#f3f4f6',
        svg: `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="#374151"><path stroke-linecap="round" stroke-linejoin="round" d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 11.953 0 0112 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0121 12c0 .778-.099 1.533-.284 2.253M3 12a8.959 8.959 0 01.284-2.253"/></svg>`
      }
    ].filter(l => l.url);
      ...
      socialContainer.innerHTML = links.map(l => `
        <a href="${l.url}" target="_blank" rel="noopener noreferrer"
          ...
          style="
            ...
            background:${l.bg};border:1px solid rgba(0,0,0,0.06);
            text-decoration:none;font-size:13px;font-weight:600;
            color:${l.color};transition:transform 0.15s;
          "
...
async function loadSucursalesPublic(storeId) {
  try {
    const res = await fetch(`/api/stores/${storeId}/sucursales`);
    if (!res.ok) return;
    ...
    container.innerHTML = sucursales.map(s => {
      const phone = s.phone || storeData?.phone;
      const waLink = phone
              ...
              ${waLink ? `
                <a href="${waLink}" target="_blank" rel="noopener noreferrer"
                  style="
                    ...
                    background:#25d366;color:white;padding:7px 14px;
              ...
              ${s.lat && s.lng ? `
                <a href="https://www.google.com/maps?q=${s.lat},${s.lng}"
                  ...
                  style="
                    ...
                    color:#374151;padding:7px 14px;
                    border-radius:8px;font-size:12px;font-weight:700;
                    text-decoration:none;
                  ">
                  <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="#ea580c">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/>
                    <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 0 1 15 0Z"/>
                  </svg>
                  Ver en mapa
